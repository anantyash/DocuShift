...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleFilesSelected(newFiles) {
    ...
    newFiles.forEach(file => {
      ...
      if (!validExtensions.includes(ext)) {
        showToast(`Skipped "${file.name}": Only .docx and .doc formats supported.`, 'error');
        return;
      }

      if (file.size > 50 * 1024 * 1024) {
        showToast(`Skipped "${file.name}": File size exceeds 50MB limit.`, 'error');
        return;
      }
    ...
    if (addedCount > 0) {
      showToast(`Added ${addedCount} document${addedCount > 1 ? 's' : ''} to queue.`, 'success');
      renderQueue();
    }
  }
    ...
    document.querySelectorAll('.preview-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (item && item.resultData) {
          openPreviewModal(item.resultData.previewUrl, item.resultData.pdfFilename, item.resultData.downloadUrl);
        }
      });
    });
  ...
  function updateActionButtonsVisibility() {
    const successCount = filesQueue.filter(f => f.status === 'success').length;
    const pendingCount = filesQueue.filter(f => f.status === 'pending' || f.status === 'error').length;
    ...
    if (pendingCount > 0) {
      ...
      convertAllBtn.querySelector('span').textContent = successCount > 0 ? 'Convert Remaining' : 'Convert All to PDF';
    } else {
      convertAllBtn.classList.add('hidden');
    }
  ...
  async function startBatchConversion() {
    const pendingItems = filesQueue.filter(f => f.status === 'pending' || f.status === 'error');
    ...
    try {
      ...
      if (data.results && Array.isArray(data.results)) {
        ...
        if (succeeded > 0) {
          showToast(`Converted ${succeeded} document${succeeded > 1 ? 's' : ''} successfully!`, 'success');
        } else {
          showToast('Conversion failed for uploaded documents.', 'error');
        }
      } else {
        throw new Error(data.error || 'Conversion server returned an invalid response.');
      }
  ...
  async function handleDownloadZip() {
    const successItems = filesQueue.filter(f => f.status === 'success' && f.resultData);
